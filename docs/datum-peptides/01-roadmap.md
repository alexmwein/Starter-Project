...
- Define events: `search_started`, `search_result_selected`, `category_selected`, `filter_applied`, `sort_changed`, `product_viewed`, `testing_opened`, `demo_add`, `cart_opened`, `cart_quantity_changed`, `cart_cleared`, `article_viewed`.
- Do not collect health, use-case, or sensitive research details.
- Create funnel and viewport dashboards.
...
- Publish the first four product-finding and evidence-literacy guides.
...
- Eight source-backed articles live.
- Every page has a defined search intent and next action.
...
- Test the Datum name, value proposition, and category language for five-second comprehension.
...
- Brand comprehension meets the internal target.
