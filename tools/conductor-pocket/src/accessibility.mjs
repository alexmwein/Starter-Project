...
export class AccessibilityTransport {
  ...
  send({
    ...
    if (!normalized.trim()) {
      return Promise.resolve({ ok: false, code: 'message_empty' });
    }
    if (normalized.includes('\0')) {
      return Promise.resolve({ ok: false, code: 'message_invalid' });
    }
    if (byteLength(normalized) > MAX_MESSAGE_BYTES) {
      return Promise.resolve({ ok: false, code: 'message_too_large' });
    }
    ...
    if (replaceDraft && normalizedExpectedDraft === null) {
      return Promise.resolve({ ok: false, code: 'draft_recheck_required' });
    }
    if (
    ...
    try {
      const { stdout } = await execFileAsync('/usr/bin/osascript', [scriptPath], {
        ...
        env: {
          ...
          POCKET_MESSAGE: message,
          POCKET_REPLACE_DRAFT: replaceDraft ? 'true' : 'false',
          POCKET_EXPECTED_DRAFT: expectedMacDraft,
        },
      });
